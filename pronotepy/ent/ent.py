...
cas_ecollege_haute_garonne = partial(
...
cas_entcreuse = partial(_cas, url="https://cas.entcreuse.fr/login")

cas_occitanie_toulouse = partial(
    _cas, url="https://cas.mon-ent-occitanie.fr/login?selection=TOULO-ENT_parent_eleve"
)

cas_occitanie_montpellier = partial(
    _cas, url="https://cas.mon-ent-occitanie.fr/login?selection=CSES-ENT_parent_eleve"
)

cas_val_doise = partial(_cas, url="https://cas.moncollege.valdoise.fr/login")

"""CAS with EduConnect"""

cas_ac_orleans_tours = partial(
...
cas_eclat_bfc = partial(_cas_edu, url="https://cas.eclat-bfc.fr/login?selection=EDU")

cas_ent_auvergnerhonealpe = partial(
    _cas_edu, url="https://cas.ent.auvergnerhonealpes.fr/login?selection=EDU"
)

cas_monbureaunumerique = partial(
    _cas_edu, url="https://cas.monbureaunumerique.fr/login?selection=EDU"
)

cas_ac_reims = cas_monbureaunumerique

cas_occitanie_montpellier_educonnect = partial(
    _cas_edu,
    url="https://cas.mon-ent-occitanie.fr/login?selection=MONT-EDU_parent_eleve",
)

ent_laclasse_educonnect = partial(
    _cas_edu, url="https://www.laclasse.com/sso/educonnect"
)

"""Open ENT NG"""

ent_77 = partial(_open_ent_ng, url="https://ent77.seine-et-marne.fr/auth/login")
...
ent_ile_de_france = partial(_open_ent_ng, url="https://ent.iledefrance.fr/auth/login")
neoconnect_guadeloupe = partial(
    _open_ent_ng, url="https://neoconnect.opendigitaleducation.com/auth/login"
)

ent_paris_classe_numerique = partial(
...
ent_l_normandie = partial(_open_ent_ng_edu, domain="https://ent.l-educdenormandie.fr")

ent_lyceeconnecte_aquitaine = partial(
...
ent_ozecollege_yvelines = partial(_oze_ent, url="https://ozecollege.yvelines.fr/")

"""Simple Auth"""

ent_atrium_sud = partial(
    _simple_auth,
    url="https://www.atrium-sud.fr/connexion/login",
    form_attr={"id": "fm1"},
)
ent_laclasse_lyon = partial(_simple_auth, url="https://www.laclasse.com/sso/login")
